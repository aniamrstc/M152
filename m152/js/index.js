...
function show(idMedia, idPost) {
    ...
    for (let i = 0; i < myText.length; i++) {
        ...
        if (typeMedia == "video") {
            media += '<video class="img-responsive" style=" width:400px;" autoplay loop> <source src=\"GetImage.php?file=' + myText[i]['idMedia'] + '\"> </video>';
        } else if (typeMedia == "audio") {
            media += ' <audio controls style="width:100%"><source src=\"GetImage.php?file=' + myText[i]['idMedia'] + '\"></audio>';

        } else {
            media += ' <img src=\""GetImage.php?file="' + myText[i]['idMedia'] + '\"  class="img-responsive" style=" width:400px;"></img>';
        }
